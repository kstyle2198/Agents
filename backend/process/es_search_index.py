...
class ElasticsearchIndexer:
    ...
    def __init__(self, es_url: str = "http://localhost:9200", index_name: Union[str, List[str]] = "open_paper"):
        """
        index_name을 문자열 또는 문자열 리스트로 받을 수 있도록 개선
        """
        # index_name이 문자열이면 리스트로 감싸서 저장
        if isinstance(index_name, str):
            self.index_names = [index_name]
        elif isinstance(index_name, list):
            self.index_names = index_name
        else:
            raise ValueError("index_name must be a string or a list of strings.")
    ...
    def search_documents(self, query_text: str = None, size: int = 10, min_score: float = 0.5):
        """
        Elasticsearch에서 텍스트 또는 임베딩 기반 문서 검색.
        여러 인덱스에 대해 동시에 검색 가능.
        """
        if not query_text:
            logger.warning("query_text must be provided.")
            return []
        ...
        try:
            # 🔥 여러 인덱스를 동시에 검색 (Elasticsearch 기본 지원)
            res = self.es.search(
                index=self.index_names,   # 리스트를 그대로 넣으면 여러 인덱스 검색됨
                body=search_body
            )
            ...
            logger.info(f"Found {len(documents)} documents from indices: {self.index_names}")
            return documents
...
es = ElasticsearchIndexer(index_name=["ai_paper", "open_paper"])
